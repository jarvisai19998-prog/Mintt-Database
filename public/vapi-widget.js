var VAPI_PUBLIC_KEY='6cd90e01-999b-472f-8cfd-900f14b9502c';
var VAPI_ASSISTANT_ID='d168f7bf-c494-4196-9fb9-6f48ecf74ed1';
...
(function(){
  ...
  st.textContent=[
    ...
    '#elp-ring1{position:absolute;inset:-8px;border-radius:50%;border:1px solid rgba(232,80,10,0.3);animation:elp-ring 2s ease-in-out infinite;}',
    '#elp-ring2{position:absolute;inset:-18px;border-radius:50%;border:1px solid rgba(232,80,10,0.15);animation:elp-ring 2s ease-in-out infinite 0.4s;}',
    '#elp-fab{width:64px;height:64px;border-radius:50%;background:linear-gradient(135deg,#e8500a,#c94008);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 8px 32px rgba(232,80,10,0.4),0 2px 8px rgba(0,0,0,0.2);position:relative;z-index:1;transition:transform .2s;}',
    ...
    '.elp-icon{width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,#e8500a,#c94008);display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 4px 12px rgba(232,80,10,0.35);}',
    ...
    '.elp-status-icon{width:32px;height:32px;border-radius:8px;background:rgba(232,80,10,0.15);display:flex;align-items:center;justify-content:center;flex-shrink:0;}',
    ...
    '.elp-bubble.user{background:linear-gradient(135deg,#e8500a,#c94008);color:#fff;}',
    ...
    '#elp-start{width:100%;height:48px;border-radius:12px;background:linear-gradient(135deg,#e8500a,#c94008);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;font-size:14px;font-weight:600;color:#fff;letter-spacing:-.01em;font-family:"Inter",sans-serif;box-shadow:0 4px 20px rgba(232,80,10,0.3);transition:all .2s;}',
    '#elp-start:hover{box-shadow:0 6px 28px rgba(232,80,10,0.45);transform:translateY(-1px);}',
  ...
  wrap.innerHTML=[
    ...
    '<button id="elp-fab"><svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg></button>',
  ...
  modal.innerHTML=[
    '<div class="elp-header">',
      '<div class="elp-brand">',
        '<div class="elp-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg></div>',
        '<div><div class="elp-name">Unitech Controls AI</div><div class="elp-live"><span class="elp-live-dot"></span>Available 24/7</div></div>',
      '</div>',
      '<button class="elp-close" id="elp-close">✕</button>',
    '</div>',
    '<div class="elp-divider"></div>',
    '<div class="elp-status">',
      '<div class="elp-status-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#e8500a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></svg></div>',
    ...
    '<div class="elp-footer">',
      '<button id="elp-start"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81a19.79 19.79 0 01-3.07-8.63A2 2 0 012 .18h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L6.09 7.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 14v2.92z"/></svg>Start AI Call</button>',
      ...
      '<div class="elp-note">Powered by ElectricLead Pro</div>',
    '</div>'
  ].join('');
  ...
  function addMsg(role,text){
    ...
    lbl.textContent=role==='ai'?'Unitech AI':'You';
  ...
  document.getElementById('elp-start').onclick=function(){
    ...
    import('https://esm.sh/@vapi-ai/web').then(function(module){
      ...
      vapi.on('call-end',function(){
        ...
        document.getElementById('elp-start').disabled=false;
        document.getElementById('elp-start').innerHTML='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81a19.79 19.79 0 01-3.07-8.63A2 2 0 012 .18h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L6.09 7.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 14v2.92z"/></svg>Start AI Call';
        document.getElementById('elp-fab').classList.remove('active');
        hideTyping();
        setStatus('Call complete','Our team will follow up shortly');
        addMsg('ai','Thanks for calling Unitech Controls! Our team will be in touch very soon.');
      });
